...
func TestSaveClonesAndCursorIsStable(t *testing.T) {
	...
	s.Save(quota.QuotaGrant{ID:"a",Tags:tags,UpdatedAt:now}); s.Save(quota.QuotaGrant{ID:"b",UpdatedAt:now}); tags["team"]="b"
...
func TestOrderingCutoffAndAtomicUpdate(t *testing.T) {
	...
	s.Save(quota.QuotaGrant{ID:"b",State:"new",UpdatedAt:now}); s.Save(quota.QuotaGrant{ID:"a",UpdatedAt:now})
	...
	want:=errors.New("stop"); if err:=s.Update("b",func(v *quota.QuotaGrant)error{v.State="done";return want}); !errors.Is(err,want){t.Fatalf("wrong update error: %v",err)}
